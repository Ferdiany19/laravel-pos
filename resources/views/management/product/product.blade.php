            ...
            <div class="row">
                            ...
                            <tbody>
                                @forelse ($categorys as $category)
                                    <tr>
                                        ...
                                        <td>{{ $category->name }}</td>
                                        <td>
                                            <form action="{{ route('management.category.destroy',$category->id) }}" method="POST" enctype="multipart/form-data">
                                                @csrf
                                                @method('DELETE')
                            ...
                            <tbody>
                                @forelse ($units as $unit)
                                    <tr>
                                        ...
                                        <td>{{ $unit->name }}</td>
                                        <td>
                                            <form action="{{ route('management.unit.destroy',$unit->id) }}" method="POST" enctype="multipart/form-data">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="btn btn-danger">DELETE</button>
                                            </form>
                ...
                <div class="col-12">
                    <div class="d-flex bg-white pb-2 pt-2 rounded-top">
                        ...
                        <a href="{{ route('management.item.create') }}" class="btn btn-primary ml-auto mb-auto mt-auto mr-2"><i class="zmdi zmdi-plus"></i> ADD ITEM</a>
                    </div>
                    <div class="bg-white rounded-bottom">
                        <div class="row mx-auto pb-2">
                            <div class="col-md-6 mb-1">
                                <a class="btn btn-danger form-control" href="{{ route('management.product.allBarcode.pdf') }}" target="_blank">Export ALL Barcode PDF <i class="fas fa-file-pdf"></i></a>
                            </div>
                            <div class="col-md-6 mb-1">
                                <a class="btn btn-danger form-control" href="{{ route('management.product.pdf') }}" target="_blank">Export Product PDF <i class="fas fa-file-pdf"></i></a>
                            </div>
                            <div class="col-12">
                                <a class="btn btn-success form-control" href="{{ route('management.product.excel') }}" target="_blank">Export Product Excel <i class="fas fa-file-excel"></i></a>
                            </div>
                        </div>
                    </div>
                    <div class="table-responsive table--no-card m-b-30">
                            ...
                            <tbody>
                                @forelse ($items as $item)
                                    <tr>
                                        ...
                                        <td>
                                            <a href="{{ route('management.item.show',$item->id) }}" class="btn btn-warning">Edit</a>
                                            <form action="{{ route('management.item.destroy',$item->id) }}" method="POST" enctype="multipart/form-data" class="d-inline-block">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="btn btn-danger">Delete</button>
                                            </form>
...
<div class="modal fade" id="modalAddCategory" tabindex="-1" role="dialog" aria-labelledby="scrollmodalLabel" aria-hidden="true" style="z-index: ">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            ...
            <form action="{{ route('management.category.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
...
<div class="modal fade" id="modalAddUnit" tabindex="-1" role="dialog" aria-labelledby="scrollmodalLabel" aria-hidden="true" style="z-index: ">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            ...
            <form action="{{ route('management.unit.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="modal-body">
                    <div class="form-group">
