            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="card-title">
                        <h3 class="text-center title-2">Edit Stock</h3>
                    </div>
                    <form action="{{ route('management.transaction.stockIn.update',$stock->id) }}" method="post" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="row">
                            <div class="form-group col-sm-6">
                                ...
                                <select name="name" id="name" class="form-control">
                                    ...
                                    @forelse ($items as $item)
                                        <option value="{{ $item->id }}" {{ $stock->items()->first()->id == $item->id ? 'selected' : ''}}>{{ $item->name }}</option>
                                    @empty
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="stock" id="stock" class="form-control" value="{{ $stock->stock }}">
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <input type="date" name="expire" id="expire" class="form-control" value="{{ $stock->expire->format('Y-m-d') }}">
                        </div>
                        <div>
