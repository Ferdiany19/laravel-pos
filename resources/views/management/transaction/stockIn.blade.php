...
@endsection
@section('btn_add')
<a href="{{ route('management.transaction.stockIn.create') }}" class="au-btn au-btn-icon au-btn--blue">
...
<div class="mt-3 row">
    <div class=" col-md-6 mb-1">
        <a class="btn btn-danger form-control" href="{{ route('management.transaction.stockIn.pdf') }}" target="_blank">Export Stock In PDF <i class="fas fa-file-pdf"></i></a>
    </div>
    <div class=" col-md-6 mb-1">
        <a class="btn btn-success form-control" href="{{ route('management.transaction.stockIn.excel') }}" target="_blank">Export Stock In Excel <i class="fas fa-file-excel"></i></a>
    </div>
</div>
<div class="mt-2">
            ...
            <tbody>
                @forelse ($items as $item)
                    ...
                    <tr>
                        ...
                        <td>
                            <a href="{{ route('management.transaction.stockIn.show',$stock->id) }}" class="btn btn-info">Edit</a>
                            <button type="button" class="btn btn-warning" data-toggle="modal" data-target="#retur{{ $stock->id }}">Retur</button>
                            <form action="{{ route('management.transaction.stockIn.destory',$stock->id) }}" method="POST" enctype="multipart/form-data" class="d-inline-block">
                                @csrf
                                @method('DELETE')
            ...
            <div class="modal fade" id="retur{{ $stock->id }}" tabindex="-1" role="dialog" aria-labelledby="scrollmodalLabel" aria-hidden="true">
                <div class="modal-dialog modal-lg" role="document">
                    <div class="modal-content">
                        ...
                        <form action="{{ route('management.transaction.stockRetur.store.stock.in',$stock->id) }}" method="POST" enctype="multipart/form-data">
                            @csrf
