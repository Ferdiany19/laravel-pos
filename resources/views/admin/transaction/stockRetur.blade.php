...
<div class="mt-3 row">
    <div class="col-md-6 mb-1">
        <a class="btn btn-danger form-control" href="{{ route('admin.transaction.stockRetur.pdf') }}" target="_blank">Export Stock Retur PDF <i class="fas fa-file-pdf"></i></a>
    </div>
    <div class="col-md-6 mb-1">
        <a class="btn btn-success form-control" href="{{ route('admin.transaction.stockRetur.excel') }}" target="_blank">Export Stock Retur Excel <i class="fas fa-file-excel"></i></a>
    </div>
</div>
<div class="mt-2">
                    ...
                    <tr>
                        ...
                        <td>
                            @if ($stock_retur->pengecekan !== 'p')
                                <form action="{{ route('admin.transaction.stockRetur.destroy',$stock_retur->id) }}" method="POST" enctype="multipart/form-data" class="d-inline-block">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="btn btn-danger">Delete</button>
                                </form>
                            @else
                            <form action="{{ route('admin.transaction.stockRetur.proses.update',$stock_retur->id) }}" method="POST" enctype="multipart/form-data" class="d-inline-block">
                                @csrf
                                @method('PUT')
