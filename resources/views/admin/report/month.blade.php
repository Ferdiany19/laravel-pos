...
$(document).on('click','#search-day',function(){
    ...
    `);
    $.ajax({
        url : "{{ route('admin.report.month.search') }}",
        method: 'post',
        dataType: "json",
        data: {
            _token : _token,
...
$(document).on('click','#pdf',function(){
    let search_month = $('[name="search_month"]').val();
    search_month = search_month.split('/').join('-');
    window.open(`{{ route("admin.report.month.index",) }}/pdf/${search_month}`)
})

$(document).on('click','#excel',function(){
    let search_month = $('[name="search_month"]').val();
    search_month = search_month.split('/').join('-');
    window.open(`{{ route("admin.report.month.index",) }}/excel/${search_month}`)
})
</script>
@endsection