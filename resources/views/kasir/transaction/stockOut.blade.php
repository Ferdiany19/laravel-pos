...
<div class="mt-3 row">
    <div class=" col-md-6 mb-1">
        <a class="btn btn-danger form-control" href="{{ route('kasir.transaction.stockOut.pdf') }}" target="_blank">Export Stock Out PDF <i class="fas fa-file-pdf"></i></a>
    </div>
    <div class=" col-md-6 mb-1">
        <a class="btn btn-success form-control" href="{{ route('kasir.transaction.stockOut.excel') }}" target="_blank">Export Stock Out Excel <i class="fas fa-file-excel"></i></a>
    </div>
</div>
<div class="mt-2">
                ...
                @forelse ($orders as $order)
                    <tr>
                        ...
                        <tr>
                            ...
                            <td>
                                <a href="{{ url('kasir/transaction/view', $order_detail->id) }}" target="_blank" class="btn btn-sm btn-flat btn-warning">Print<i class="fa fa-eye"></i></a>
                                <button type="button" class="btn btn-warning" data-toggle="modal" data-target="#retur{{ $order_detail->id }}">Retur</button>
                                <form action="{{ route('kasir.transaction.stockOut.destroy',$order_detail->id) }}" method="POST" enctype="multipart/form-data" class="d-inline-block">
                                @csrf
                                @method('DELETE')
    ...
    <div class="modal fade" id="retur{{ $order_detail->id }}" tabindex="-1" role="dialog" aria-labelledby="scrollmodalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                ...
                <form action="{{ route('kasir.transaction.stockRetur.store.stock.out',$order_detail->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
